"""Dev entry point:  python run.py   ->  http://127.0.0.1:8010

Port 8010, not 8000, so the interviewer and the screening app can run together.

Builds the React UI (frontend/, Vite - two pages: the recruiter console and the
candidate's own interview) if it is missing or out of date, then serves it from
the FastAPI app - so a single `python run.py` is all you need.
...
# public/ holds the avatar rigs, speech.js and three.js, copied into the build
# verbatim - a change to any of them needs a rebuild just as much as src/ does.
